...
import { DEFAULT_AVG_PASS_LENGTH_CONFIG, getChartTitle } from '../types/chartConfig';
...
export const AvgPassLengthChart: React.FC<AvgPassLengthChartProps> = ({
  ...
  const { config, isLoading, handleConfigChange, handleSave, handleReset, handleExpandToggle, isExpanded } = useChartConfig({
    chartType: 'avgPassLength',
    defaultConfig: DEFAULT_AVG_PASS_LENGTH_CONFIG,
    globalIncludeOpponents,
    onExpansionChange,
  });
  ...
  const chartData = data.map((match) => {
    ...
    if (config.visibleMetrics.includes('avgPassLength')) {
      base['Avg Pass Length'] = typeof match[avgPassLengthKey] === 'number' ? match[avgPassLengthKey] : null;
    }

    // Add opponent metrics when includeOpponent is true
    if (config.includeOpponent && config.visibleMetrics.includes('avgPassLength')) {
      base['Opp Avg Pass Length'] = typeof match[oppAvgPassLengthKey] === 'number' ? match[oppAvgPassLengthKey] : null;
    }
  ...
  const renderBars = () => {
    ...
    if (config.visibleMetrics.includes('avgPassLength')) {
      bars.push(
        <Bar key="Avg Pass Length" dataKey="Avg Pass Length" fill={JOGA_COLORS.voltYellow} animationDuration={500}>
    ...
    if (config.includeOpponent && config.visibleMetrics.includes('avgPassLength')) {
      bars.push(
        <Bar key="Opp Avg Pass Length" dataKey="Opp Avg Pass Length" fill={OPPONENT_COLORS.primary} animationDuration={500}>
  ...
  const availableMetrics = [
    { id: 'avgPassLength', label: 'Average Pass Length', required: false },
  ];

  // Generate dynamic title
  const chartTitle = getChartTitle('avgPassLength', config.visibleMetrics);
