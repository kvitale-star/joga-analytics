...
import { DEFAULT_ATTEMPTS_CONFIG, getChartTitle } from '../types/chartConfig';
...
export const AttemptsChart: React.FC<AttemptsChartProps> = ({
  data,
  attemptsKey,
  oppAttemptsKey,
  opponentKey,
  ...
  const { config, isLoading, handleConfigChange, handleSave, handleReset, handleExpandToggle, isExpanded } = useChartConfig({
    chartType: 'attempts',
    defaultConfig: DEFAULT_ATTEMPTS_CONFIG,
    globalIncludeOpponents,
    onExpansionChange,
  });
  ...
  const chartData = data.map((match) => {
    ...
    if (config.visibleMetrics.includes('attemptsFor')) {
      base['Attempts'] = typeof match[attemptsKey] === 'number' ? match[attemptsKey] : 0;
    }
  ...
  const renderBars = () => {
    ...
    if (config.visibleMetrics.includes('attemptsFor')) {
      bars.push(
        <Bar key="Attempts" dataKey="Attempts" fill={JOGA_COLORS.voltYellow} animationDuration={500}>
          {showLabels && <LabelList dataKey="Attempts" position="top" fill="#666" fontSize={12} />}
        </Bar>
      );
    }
    if (config.visibleMetrics.includes('shotsFor') && shotsForKey) {
    ...
    if (config.includeOpponent) {
      if (config.visibleMetrics.includes('attemptsFor')) {
        bars.push(
          <Bar key="Opp Attempts" dataKey="Opp Attempts" fill={OPPONENT_COLORS.primary} animationDuration={500}>
