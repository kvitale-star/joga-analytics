...
export const ConversionRateChart: React.FC<ConversionRateChartProps> = ({
  ...
  const chartData = data.map((match) => ({
    ...
    'Conversion Rate': typeof match[conversionRateKey] === 'number' ? match[conversionRateKey] : 0,
    'Opp Conversion Rate': typeof match[oppConversionRateKey] === 'number' ? match[oppConversionRateKey] : 0,
  }));

  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h3 className="text-xl font-bold mb-2 text-gray-800">Conversion Rate</h3>
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={chartData} margin={{ top: 5, right: 30, left: 30, bottom: 10 }}>
          ...
          />
          <Bar dataKey="Conversion Rate" fill={JOGA_COLORS.voltYellow} animationDuration={500}>
            {showLabels && <LabelList dataKey="Conversion Rate" position="top" fill="#666" fontSize={12} formatter={(value: number) => `${value.toFixed(1)}%`} />}
          </Bar>
          <Bar dataKey="Opp Conversion Rate" fill={OPPONENT_COLORS.primary} animationDuration={500}>
